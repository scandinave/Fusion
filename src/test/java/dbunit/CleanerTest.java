...
public class CleanerTest {
	...
	// private AbstractPostgreSQLWorker worker;
	...
	// worker = Mockito.mock(AbstractPostgreSQLWorker.class);
	...
	// Set<RowLiquibaseDatabasechangelogBDD> rows = new
	// HashSet<RowLiquibaseDatabasechangelogBDD>();
	...
	//
	// for (int i = 0; i < 10; i++) {
	// RowLiquibaseDatabasechangelogBDD rowLiquid =
	// Mockito.mock(RowLiquibaseDatabasechangelogBDD.class);
	// Mockito.when(rowLiquid.getRowXML()).thenReturn(new
	// Row("liquibase.databaseChangelog", attributs));
	// rows.add(rowLiquid);
	// }
	//
